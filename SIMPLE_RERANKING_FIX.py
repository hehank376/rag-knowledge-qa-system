...
直接修复现有重排序服务的配置读取问题，支持API调用，
不进行复杂的架构重构。
"""

SIMPLE_RERANKING_SERVICE = '''
"""
重排序服务模块
...
def apply_simple_fix():
    ...
    # 写入修复后的重排序服务
    with open('rag_system/services/reranking_service.py', 'w', encoding='utf-8') as f:
        f.write(SIMPLE_RERANKING_SERVICE)
    
    print("✅ 重排序服务修复完成")
    print("\n📋 修复内容:")
    print("   1. 修复配置读取逻辑 - 支持 model_name 和 model 两种键名")
    print("   2. 添加API调用支持 - 支持SiliconFlow等API提供商")
    print("   3. 保持本地模型支持 - 兼容sentence-transformers")
    print("   4. 简化架构 - 移除复杂的工厂模式和基类")
    print("   5. 保持API兼容性 - 确保前端功能正常工作")
    
    print("\n🎯 现在前端应该能够:")
    print("   ✅ 正确读取配置文件中的重排序模型配置")
    print("   ✅ 保存重排序模型配置")
    print("   ✅ 测试重排序模型连接")
    print("   ✅ 切换活跃重排序模型")
    print("   ✅ 查看重排序模型状态和指标")
