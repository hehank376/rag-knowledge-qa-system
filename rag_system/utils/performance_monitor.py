...
class SystemStabilityStats:
    ...
    def calculate_stability_score(self):
        ...
        if self.memory_usage_percent > 85:
            score -= (self.memory_usage_percent - 85) * 3
...
class RetrievalPerformanceMonitor:
    ...
    def __init__(self, max_metrics_history: int = 10000):
        ...
        # 启动系统监控（在测试环境中禁用）
        import os
        if os.environ.get('DISABLE_SYSTEM_MONITORING') != 'true':
    ...
    def _start_system_monitoring(self):
        ...
        def monitor_system_resources():
            try:
                # 获取系统资源使用情况（使用非阻塞方式）
                self.system_stability.cpu_usage_percent = psutil.cpu_percent(interval=None)
                self.system_stability.memory_usage_percent = psutil.virtual_memory().percent
                ...
                # 设置默认值
                self.system_stability.cpu_usage_percent = 0.0
                self.system_stability.memory_usage_percent = 0.0
...
def get_global_monitor():
    ...
    if global_performance_monitor is None:
        global_performance_monitor = RetrievalPerformanceMonitor()
    return global_performance_monitor


# 便捷函数
def start_performance_monitoring(query_id: str, operation_type: OperationType, 
                               search_mode: Optional[SearchMode] = None,
                               metadata: Optional[Dict[str, Any]] = None) -> str:
    """开始性能监控的便捷函数"""
    return get_global_monitor().start_operation(query_id, operation_type, search_mode, metadata)


def complete_performance_monitoring(metric_key: str, success: bool = True, 
                                  result_count: int = 0, error_details: Optional[str] = None):
    """完成性能监控的便捷函数"""
    get_global_monitor().complete_operation(metric_key, success, result_count, error_details)


def record_cache_hit(hit: bool, duration_ms: float):
    """记录缓存命中的便捷函数"""
    get_global_monitor().record_cache_operation(hit, duration_ms)


def record_reranking_performance(success: bool, duration_ms: float, document_count: int = 0):
    """记录重排序性能的便捷函数"""
    get_global_monitor().record_reranking_operation(success, duration_ms, document_count)


def record_search_mode_performance(mode: SearchMode, duration_ms: float, success: bool):
    """记录搜索模式性能的便捷函数"""
    get_global_monitor().record_search_mode_usage(mode, duration_ms, success)


def log_performance_exception(exception: Exception, context: Dict[str, Any]):
    """记录性能相关异常的便捷函数"""
    get_global_monitor().log_exception_details(exception, context)


def get_performance_report(time_range_minutes: int = 60) -> PerformanceReport:
    """获取性能报告的便捷函数"""
    return get_global_monitor().get_detailed_performance_stats(time_range_minutes)


# 性能监控装饰器
def monitor_performance(operation_type: OperationType, search_mode: Optional[SearchMode] = None):
    """性能监控装饰器"""
    def decorator(func):
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                complete_performance_monitoring(metric_key, success=True, result_count=result_count)
                return result
            except Exception as e:
                complete_performance_monitoring(metric_key, success=False, error_details=str(e))
                log_performance_exception(e, {"function": func.__name__, "args": str(args)[:100]})
                raise
